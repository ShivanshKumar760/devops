...
import express from "express";
import bcrypt from "bcryptjs"; // Changed to bcryptjs to fix Docker compilation issues
import pool from "./db/db.js";
import { createToken, tokenRequired } from "./middleware/auth.js";
import cors from "cors";
...
app.get("/health", async (req, res) => {
    try {
        // Quick check to make sure database is also responsive
        await pool.query("SELECT 1");
...
app.post("/signup", async (req, res) => {
    ...
    try {
        ...
        // FIXED: Changed 'err.code' to 'error.code' to prevent server crash
        if (error.code === "23505") {
            return res.status(409).json({ error: "username or email already taken" });
        }
        console.error(error);
...
app.post("/posts", tokenRequired, async (req, res) => {
    ...
    const result = await pool.query(
        `INSERT INTO posts (user_id, caption, image_url)
        VALUES ($1, $2, $3)
        RETURNING post_id, user_id, caption, image_url, created_at`, [req.userId, caption, image_url]
    );
...
// ---------- Database Connection with Automatic Retries ----------
async function connectWithRetry(maxRetries = 5, delayMs = 3000) {
    for (let i = 1; i <= maxRetries; i++) {
        try {
            ...
            console.error(`⚠️ Database connection attempt ${i} failed. Retrying in ${delayMs / 1000}s...`);
            if (i === maxRetries) {
                console.error("❌ CRITICAL: Could not connect to database after maximum attempts.");
                // Optional: process.exit(1); // Stop the server if DB is required to function
            }
            await sleep(delayMs);
        }
    }
...
// Call the retry connection handler
await connectWithRetry();

const PORT = 5001;
const server = app.listen(PORT, () => {
    console.log(`Server running on http://localhost:${PORT}`);
    // FIXED: Placed inside the callback so it correctly prints true
    console.log("Server listening active:", server.listening);
});
